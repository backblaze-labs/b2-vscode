...
| Harness (scripts / CI)            | A     | Extensive `assert-*` gates, audit policy, VSIX + release contracts, `npm run check` mirrors CI       | Layering now enforced (`assert-layers.js`); no doc-drift gate yet ([TD-3](exec-plans/tech-debt-tracker.md#td-3))                                                                         |
